...
public class GHJOperator extends JoinOperator {
    ...
    private void buildAndProbe(Partition leftPartition, Partition rightPartition) {
        ...
        // Building stage
        for (Record b: buildRecords) {
            DataBox value;
            value = b.getValue(buildColumnIndex);
            if (!hashTable.containsKey(value)) {
                hashTable.put(value, new ArrayList<>());
            }
            hashTable.get(value).add(b);
        }

        // Probing stage
        for (Record p: probeRecords) {
            DataBox value = p.getValue(probeColumnIndex);
            ...
            for (Record lRecord : hashTable.get(value)) {
                Record joinedRecord;
                if (probeFirst) {   //left
                    joinedRecord = p.concat(lRecord);
                }
                else {          //right
                    joinedRecord = lRecord.concat(p);
                }
