...
public class SortOperator extends QueryOperator {
    ...
    public Run sortRun(Iterator<Record> records) {
        ...
        Run ret = new Run(this.transaction, this.getSchema());
        List<Record> li = new ArrayList<>();
        while (records.hasNext()) {
            li.add(records.next());
        }
        Collections.sort(li, this.comparator);
        ret.addAll(li);
        return ret;
    }
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        List<Iterator> li = new ArrayList<Iterator>();
        for (Run r : runs) {
            li.add(r.iterator());
        }
        boolean x = true;
        for (int i = 0; i < li.size(); i++) {           //add the first elements to pq
            if (li.get(i).hasNext()) {
                pq.add(new Pair<Record, Integer>((Record) li.get(i).next(), i));
            }
        }
        while (x) {
            Pair<Record, Integer> smallest = pq.poll();
            ret.add(smallest.getFirst());
            Integer index = smallest.getSecond();
            if (li.get(index).hasNext()) {
                pq.add(new Pair<Record, Integer>((Record) li.get(index).next(), index));
            }
            x = false;
            for (Iterator l: li) {
                if (l.hasNext() || !pq.isEmpty()) {
                    x = true;
                    break;
                }
            }
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        int hardLimit = Math.floorDiv(runs.size(), (numBuffers - 1))*(numBuffers-1);
        for (int i = 0; i<hardLimit/(numBuffers-1); i++) {
            sorted.add(this.mergeSortedRuns(runs.subList(i*(numBuffers-1), (i+1)*(numBuffers-1))));
        }
        //if statement
        if (runs.size() > hardLimit) {
            sorted.add(this.mergeSortedRuns(runs.subList(hardLimit, runs.size())));
        }
        return sorted;
    }
    ...
    public Run sort() {
        ...
        // TODO(proj3_part1): implement
        List<Run> ret;
        BacktrackingIterator<Record> initial;
        List<Run> runs = new ArrayList<>();
        while (sourceIterator.hasNext()) {
            initial = getBlockIterator(sourceIterator, this.getSchema(), numBuffers);
            runs.add(sortRun(initial));
        }
        if (runs.size() == 1) {
            return runs.get(0);
        }
        ...
        return ret.get(0); // TODO(proj3_part1): replace this!
    }
