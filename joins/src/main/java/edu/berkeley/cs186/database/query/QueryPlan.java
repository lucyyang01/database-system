...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        Table t = this.transaction.getTable(table);
        ...
        for (int index : getEligibleIndexColumns(table)) {
            ...
            IndexScanOperator d = new IndexScanOperator(this.transaction, table, predicate.column, predicate.operator, predicate.value);
            int currCost = d.estimateIOCost();
            if (currCost < cost) {
                cost = currCost;
                minOp = d;
                i = index;
        ...
        minOp = addEligibleSelections(minOp, i);
        return minOp;
    }
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        for(Set<String> set: prevMap.keySet()) {
            for(JoinPredicate predicate: this.joinPredicates) {
                ...
                String rightCol = predicate.rightColumn;
                String leftTable = predicate.leftTable;
                Set<String> leftSet = new HashSet<>();
                leftSet.add(leftTable);
                String leftCol = predicate.leftColumn;
                QueryOperator x;
                if(set.contains(leftTable) && !set.contains(rightTable)) {
                    //use pass1Map to access rightTable
                    x = pass1Map.get(rightSet);       //right operator
                    Set<String> right = new HashSet<>();
                    right.addAll(set);
                    right.add(rightTable);
                    QueryOperator costs = minCostJoinType(prevMap.get(set), x, leftCol, rightCol);
                    if (!result.containsKey(right)) {
                        result.put(right, costs);
                    }
                    else {
                        if (result.get(right).estimateIOCost()>costs.estimateIOCost()) {
                            result.put(right, costs);
                        }
                    }
                }
                else if(set.contains(rightTable) && !set.contains(leftTable)){
                    //use pass1map to access leftTable
                    x = pass1Map.get(leftSet);
                    Set<String> left = new HashSet<>();
                    left.addAll(set);
                    left.add(leftTable);
                    QueryOperator costs = minCostJoinType(prevMap.get(set), x, rightCol, leftCol);
                    if (!result.containsKey(left)) {
                        result.put(left, costs);
                    }
                    else {
                        if (result.get(left).estimateIOCost()>costs.estimateIOCost()) {
                            result.put(left, costs);
                        }
                    }
                }
    ...
    public Iterator<Record> execute() {
        ...
        Map<Set<String>, QueryOperator> passOneMap = new HashMap<>();
        for (String t : this.tableNames) {
            HashSet<String> table = new HashSet<>();
            table.add(t);
            passOneMap.put(table, minCostSingleAccess(t));
        }
        int pass = 1;
        Map<Set<String>, QueryOperator> currMap = new HashMap<>(passOneMap);
        while (pass<this.tableNames.size()) {
            pass = pass + 1;
            currMap = minCostJoins(currMap, passOneMap);
        }
