...
public class TestSortMergeJoin {
    ...
    @Test
    @Category(PublicTests.class)
    public void testEmpty() {   //joining an empty table with another table should result in an iterator that returns no records
        d.setWorkMem(5); // B=5
        try(Transaction transaction = d.beginTransaction()) {
            setSourceOperators( //makes the tables
                    TestUtils.createSourceWithAllTypes(100),
                    TestUtils.createSourceWithAllTypes(0),
                    transaction
            );

            startCountIOs();    //counts number of page accesses

            JoinOperator joinOperator = new BNLJOperator(leftSourceOperator, rightSourceOperator, "int", "int",
                    transaction.getTransactionContext());   //joins the two tables

            Iterator<Record> outputIterator = joinOperator.iterator();  //makes join iterator
