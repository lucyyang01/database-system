...
public class Database implements AutoCloseable {
    ...
    private class TransactionContextImpl extends TransactionContext {
        ...
        public void close() {
            try {
                // TODO(proj4_part2)
                List<Lock> transLocks  = lockManager.getLocks(getTransaction());    //locks on a current transaction
                for (int x = transLocks.size()-1; x>=0; x--) {
                    Lock currLock = transLocks.get(x);
