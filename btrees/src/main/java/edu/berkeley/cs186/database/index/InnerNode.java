...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        ...
        //getchild, either inner node or leaf node, call get,
    ...
    public LeafNode getLeftmostLeaf() {
        ...
        // TODO(proj2): implement
        //page child node is seralized on
        //List<Long> kiddos = getChildren();
        //long pageNum = kiddos.get(0); this is calculated within getchild
        //getChild(0); //child corresponding to page number, returns node

        return getChild(0).getLeftmostLeaf();
    }

    //sync corresponds to writing to disk - mimics writing back to disk
    //if u mutate something call sync but not too many times to write back to disk
    // See BPlusNode.put.
    @Override
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        // TODO(proj2): implement
        //splitting children from 0 to d + 1
        BPlusNode curr = getChild(numLessThanEqual(key, keys));
        Optional<Pair<DataBox, Long>> returned = curr.put(key, rid);
        if (returned.equals(Optional.empty())) {
            //error here, keys.size + 1 != children.size -> when returned.equals(Optional.empty)
            sync();
            return Optional.empty();
        } else {
            DataBox copiedKey = returned.get().getFirst();
            int index = numLessThanEqual(copiedKey, keys);
            keys.add(index, copiedKey);
            children.add(index + 1, returned.get().getSecond());
            if (keys.size() > 2 * metadata.getOrder()) {
                //directly mutate keys/children or change the pointers from keys/children?
                //List<DataBox> lefthalfkeys = keys.subList(0, metadata.getOrder() + 1);
                //List<Long> lefthalfchildren = children.subList(0, metadata.getOrder());
                List<DataBox> righthalfkeys = keys.subList(metadata.getOrder() + 1, keys.size());
                List<Long> righthalfchildren = children.subList(metadata.getOrder() + 1, children.size());
                //get middle element before keys is mutated
                DataBox middleEl = keys.get(metadata.getOrder());
                keys = keys.subList(0, metadata.getOrder());
                //children list should have one more element that
                children = children.subList(0, metadata.getOrder() + 1);

                InnerNode rightNode = new InnerNode(metadata, bufferManager, righthalfkeys, righthalfchildren, treeContext);
                //InnerNode leftNode = new InnerNode(metadata, bufferManager, lefthalfkeys, lefthalfchildren, treeContext);
                //pop 0 index of right node ( new node)  to push it up

                DataBox pushed = middleEl;
                Pair<DataBox, Long> paired = new Pair(middleEl, rightNode.getPage().getPageNum());
                sync();
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        // TODO(proj2): implement
        while(data.hasNext()) {
            BPlusNode curr = getChild(children.size() - 1);
            Optional<Pair<DataBox, Long>> returned = curr.bulkLoad(data, fillFactor);
            if (returned.equals(Optional.empty())) {
                //error here, keys.size + 1 != children.size -> when returned.equals(Optional.empty)
                sync();
                return Optional.empty();
