...
public class BPlusTree {
    ...
    public Iterator<RecordId> scanAll() {
        ...
        //if null root node, then return iterator with no elements
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        //rootnode is an inner node, innernode.put handles splits

        Optional<Pair<DataBox, Long>> returned = root.put(key, rid);
        if (!returned.equals(Optional.empty())) {
            List<DataBox> rootKeys = Arrays.asList(returned.get().getFirst()); //is this legal? initialize list in one line
            //left child: original root, right child: right neighbor created by innernode
            List<Long> rootChildren = Arrays.asList(root.getPage().getPageNum(), returned.get().getSecond());
            BPlusNode newRoot = new InnerNode(metadata, bufferManager, rootKeys, rootChildren, lockContext);
            //update root syncs for us
            updateRoot(newRoot);
        }
    }
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        // the tree's root if the old root splits.
        while(data.hasNext()) {
            Optional<Pair<DataBox, Long>> returned = root.bulkLoad(data, fillFactor);
            if (!returned.equals(Optional.empty())) {
                List<DataBox> rootKeys = new ArrayList<>();//Arrays.asList(returned.get().getFirst()); //is this legal? initialize list in one line
                //left child: original root, right child: right neighbor created by innernode
                List<Long> rootChildren = new ArrayList<>();//Arrays.asList(root.getPage().getPageNum(), returned.get().getSecond());
                rootKeys.add(returned.get().getFirst());
                rootChildren.add(root.getPage().getPageNum());
                rootChildren.add(returned.get().getSecond());
                BPlusNode newRoot = new InnerNode(metadata, bufferManager, rootKeys, rootChildren, lockContext);
                //update root syncs for us
                updateRoot(newRoot);
            }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public boolean hasNext() {
            // TODO(proj2): implement
            //check if iterator over current leafnode is null
            if (this.iter.hasNext()) {
                return true;
            } else if (this.curr.getRightSibling().isPresent()){
                this.curr = this.curr.getRightSibling().get();
                this.iter = this.curr.scanAll();
                return hasNext();
            }
            return false;
        }
        ...
        @Override
        //return iterator over rids in leafnode
