...
class LeafNode extends BPlusNode {
    ...
    // See BPlusNode.put.
    //indexof, right node is the new node
    @Override
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        if (!keys.contains(key)) {
            ...
            throw new BPlusTreeException("Tree already contains this key.");
        }
        if (keys.size() <= 2 * metadata.getOrder()) {
            sync();
            return Optional.empty();
        } else {
            //split leaf node, right node is the new node, left node is the old node
            //List<DataBox> lefthalfkeys = keys.subList(0, metadata.getOrder());
            //List<RecordId> lefthalfrids = rids.subList(0, metadata.getOrder());
            List<DataBox> righthalfkeys = keys.subList(metadata.getOrder(), keys.size());
            List<RecordId> righthalfrids = rids.subList(metadata.getOrder(), rids.size());
            keys = keys.subList(0, metadata.getOrder());
            rids = rids.subList(0, metadata.getOrder());
            //rightsibling
            LeafNode rightNode = new LeafNode(metadata, bufferManager, righthalfkeys, righthalfrids, this.rightSibling, treeContext);
            //Optional<Long> newrightsibling = Optional.of(rightNode.getPage().getPageNum());
            rightSibling = Optional.of(rightNode.getPage().getPageNum());
            //LeafNode leftNode = new LeafNode(metadata, bufferManager, lefthalfkeys, lefthalfrids, newrightsibling, treeContext);
            //a pair (split_key, right_node_page_num)
            Pair<DataBox, Long> paired = new Pair(righthalfkeys.get(0), rightNode.getPage().getPageNum());
            sync();
    ...
    //cannot bulk load into an empty treee
    @Override
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
                                                  float fillFactor) {
        // TODO(proj2): implement
        //if(data.hasNext() && keys.size() < fillFactor){
        //keys.add(data.next().getFirst());
        //rids.add(data.next().getSecond());
        //sync();
        //return Optional.empty();
        //}
        //if the fillfactor hasn't been reached we can keep loading in keys
        //check for duplicate keys
        while (data.hasNext()) {
            Pair<DataBox, RecordId> dataNew = data.next();
            if(keys.contains(dataNew.getFirst())){
                throw new BPlusTreeException("no duplicate keys");
            }
            keys.add(dataNew.getFirst());
            rids.add(dataNew.getSecond());
            //left half is 2d*fillfactor
            if(keys.size() > (int) Math.ceil(fillFactor * 2 * metadata.getOrder())) {
                List<DataBox> righthalfkeys = keys.subList((int) Math.ceil(fillFactor * 2 * metadata.getOrder()), keys.size());
                List<RecordId> righthalfrids = rids.subList((int) Math.ceil(fillFactor * 2 * metadata.getOrder()), rids.size());
                keys = keys.subList(0, (int) Math.ceil(fillFactor * 2 * metadata.getOrder()));
                rids = rids.subList(0, (int) Math.ceil(fillFactor * 2 * metadata.getOrder()));
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        //this handles the error for "page -1 not found"
        Long id = buf.getLong();
        Optional<Long> rightSibling;
        if(id == -1){
            rightSibling = Optional.empty();
        } else {
            rightSibling = Optional.of(id);
        }
